...
try:
    ...
    standard_logger.warning(
        f"GCP Telemetry/Logging not initialized (Application Default Credentials missing): {e}. "
        "Falling back to standard local logging."
    )
    class StandardLoggerWrapper:
        def log_struct(self, data, severity="INFO"):
            standard_logger.info(f"[{severity}] {data}")
...
app.title = "ev-route-planner"
app.description = "API for interacting with the Agent ev-route-planner"
